...
def test_gradient_boosting(n_samples=1000, distance=0.6):
    ...
    loss6bin = losses.BinFlatnessLossFunction(uniform_features, fl_coefficient=1.0, uniform_label=0)
    loss7bin = losses.BinFlatnessLossFunction(uniform_features, fl_coefficient=1.0, uniform_label=[0, 1])
    loss6knn = losses.KnnFlatnessLossFunction(uniform_features, fl_coefficient=1.0, uniform_label=1)
    loss7knn = losses.KnnFlatnessLossFunction(uniform_features, fl_coefficient=1.0, uniform_label=[0, 1])
    ...
    for loss in [
        ...
        print(loss)
        clf = UGradientBoostingRegressor(
            loss=loss,
            max_depth=3,
            n_estimators=50,
            learning_rate=0.01,
...
def test_gb_regression(n_samples=1000):
    ...
    zeromse = 0.5 * mean_squared_error(y, y * 0.0)
