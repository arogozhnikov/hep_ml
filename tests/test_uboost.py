...
def test_cuts(n_samples=1000):
    ...
    for algorithm in ["SAMME", "SAMME.R"]:
        for target_efficiency in [0.1, 0.3, 0.5, 0.7, 0.9]:
            ...
            assert uBDT.score_cut == uBDT.score_cuts_[-1], "something wrong with computed cuts"

            for score, cut in zip(uBDT.staged_decision_function(trainX[trainY > 0.5]), uBDT.score_cuts_):
...
def test_probas(n_samples=1000):
    ...
    for algorithm in ["SAMME", "SAMME.R"]:
        uboost_classifier = uBoostClassifier(algorithm=algorithm, efficiency_steps=3, **params)
        ...
        assert np.allclose(score1, score2), "staged_score doesn't coincide with the score."
...
def test_quality(n_samples=3000):
    ...
    for algorithm in ["SAMME", "SAMME.R"]:
        uboost_classifier = uBoostClassifier(algorithm=algorithm, efficiency_steps=5, **params)
...
def check_classifiers(n_samples=10000):
    ...
    clf_dict = OrderedDict(
        {"Ada": ada, "uBOOST": uBoost_SAMME, "uBOOST.R": uBoost_SAMME_R, "uBOOST.R2": uBoost_SAMME_R_threaded}
    )
