...
def test_build_normalizer(checks=10):
    ...
    percentiles = [100 * (i + 1.0) / (checks + 1.0) for i in range(checks)]
    assert numpy.all(abs(numpy.percentile(result, percentiles) - numpy.array(percentiles) / 100.0) < 0.01)
...
def test_compute_knn_indices(n_events=100):
    X, y = generate_sample(n_events, 10, distance=0.5)
    is_signal = y > 0.5
