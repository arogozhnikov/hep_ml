...
>>> from hep_ml.splot import SPlot
>>> p = pandas.DataFrame({'signal': p_signal, 'bkg', b_bkg})
>>> sWeights = SPlot().compute_sweights(p)
...
class SPlot(object):
    ...
    def compute_sweights(self, probabilities, sample_weight=None):
        ...
        assert numpy.all(sample_weight >= 0), 'sample weight should be non-negative'
        ...
        self.V = numpy.linalg.inv(V_inv) * initial_stats[numpy.newaxis, :]

        # Final formula
        sweights = p.dot(self.V) * sample_weight[:, numpy.newaxis]
